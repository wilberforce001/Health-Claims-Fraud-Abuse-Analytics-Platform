...
providers = [random.randint(1000000000, 1999999999) for _ in range(NUM_PROVIDERS)]
...
for provider in providers:
    ...
    # Base volume (Poisson distribution for realism)
    base_claims = np.random.poisson(lam=40)

    # 5% of providers are high-risk outliers
    ...
    for _ in range(base_claims):
        ...
        # Add noise 
        claim_amount = round(np.random.normal(base_amount, base_amount * 0.15),2)

        # Fraud providers inflate billing
        if base_claims > 200:
...
cur.executemany(insert_query, claims_to_insert)
conn.commit()
